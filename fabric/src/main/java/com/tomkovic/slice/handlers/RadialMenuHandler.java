...
public class RadialMenuHandler {
    ...
    public static void register() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            } else if (KeyBindings.isMouseButton() && !KeyBindings.isOpenRadialMenuPressed()) {
                if (RadialMenuState.isMenuOpen && !clickToSelect) {
                    RadialMenuState.handleMenuToggle(false, false, isToggleEnabled,
                        () -> renderer.onMenuOpen(), () -> renderer.onMenuClose()
                    );
                }
            }
    ...
    private static void handleMenuClick(Minecraft mc) {
        boolean isMousePressed = mc.mouseHandler.isLeftPressed();
        
        
        if (clickToSelect && isMousePressed && !wasMousePressed) {
